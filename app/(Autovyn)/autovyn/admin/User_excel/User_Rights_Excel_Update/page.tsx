...
const Approver2 = () => {
  ...
  const showdata = async () => {
    try {
      window.location.href = `${process.env.NEXT_PUBLIC_URL}/users/rightsupdateformat?compcode=${user?.Comp_Code}`;
    } catch (error) {
  ...
  const columns1 = [
    ...
    { Header: "new key", accessor: "new_key" },
  ];
  ...
  const handleButtonClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/users/userrightsreportexcelupdate`,
        formData,
        {
          headers: {
      ...
      if (response.status == 200) {
        setTabledata(response.data.CorrectData);
        ...
        <div className="rounded-t bg-white dark:bg-dark px-6 py-2">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <MediumTitle text="User Rights Update Excel Import" />
            </div>
            <div className="flex items-center gap-2">
              <Button
                onClick={showdata}
