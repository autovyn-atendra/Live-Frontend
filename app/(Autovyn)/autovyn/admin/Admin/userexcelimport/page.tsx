...
const Approver2 = () => {
  ...
  const showdata = async () => {
    try {
      window.location.href = `${process.env.NEXT_PUBLIC_URL}/users/importformatuser?compcode=${user?.Comp_Code}`;
    } catch (error) {
  ...
  const columns1 = [
    ...
    { Header: "Password", accessor: "User_Pwd" },
    { Header: "Mobile No", accessor: "User_mob" },
    { Header: "Email", accessor: "User_Email" },
    { Header: "Multi location", accessor: "Multi_loc" },
    { Header: "Employee Code", accessor: "EMPCODE" },
    { Header: "Employee Dms Code", accessor: "emp_dms_code" },
  ];
  ...
  const handleButtonClick = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_URL}/users/excelimportuser`,
        formData,
        {
          headers: {
        ...
        <div className="rounded-t bg-white dark:bg-dark px-6 py-2">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <MediumTitle text="user Excel Import" />
            </div>
