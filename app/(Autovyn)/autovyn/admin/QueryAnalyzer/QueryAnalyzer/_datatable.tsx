...
const DataTable = ({
  ...
  const slicedData = React.useMemo(() => {
    const baseData = filterPosition === "FilterData" ? filteredData : data;
    return baseData?.slice(0, 100) || [];
  }, [data, filteredData, filterPosition]);

  const tableInstance = useTable(
    {
      columns,
      data: slicedData, // ✅ Now passing only top 100 rows
    ...
    <div className="p-0">
      ...
      <div className={`overflow-y-scroll ${height ? `h-[${height}]` : 'h-[400px]'} no-visible-scrollbar`}>
        ...
        >
          ...
          {/* Main table header */}
          <thead
            className={`sticky ${filterPosition === "FilterData" && enableColumnFilters ? "top-[38px]" : "top-0"} z-1 border-b border-body-color h-6 bg-white dark:bg-input text-black dark:text-white`}
                ...
                >
                  ...
                  {row.cells.map((cell) => (
                    <td
                      ...
                      className={`border  px-2 py-[3px] whitespace-nowrap dark:text-white dark:bg-input bg-[#F3F6F9]  ${cell.column.cellAlign === "right" ? "text-right" :
