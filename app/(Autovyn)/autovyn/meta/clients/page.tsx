...
export default function OnboardingClientsPage() {
  ...
  // Selected client for FULL DETAIL VIEW
  ...
  const filteredClients = clients.filter((c) => {
    ...
    let matchSla = true;
    if (filterSLA === "On track") matchSla = sla === "ontrack";
    if (filterSLA === "Due today") matchSla = sla === "due";
    if (filterSLA === "Overdue") matchSla = sla === "overdue";
    if (filterSLA === "Blocked") matchSla = sla === "blocked";
    if (filterSLA === "Live") matchSla = sla === "live";
    return matchSearch && matchOwner && matchType && matchSla;
  });

  // Summary counts
  const totalCount = clients.length;
  const onTrackCount = clients.filter((c) => !c.live && getSlaStatus(c) === "ontrack").length;
  const dueTodayCount = clients.filter((c) => !c.live && getSlaStatus(c) === "due").length;
  const overdueCount = clients.filter((c) => !c.live && getSlaStatus(c) === "overdue").length;
  const blockedCount = clients.filter((c) => !c.live && getSlaStatus(c) === "blocked").length;
  ...
  const handleCreateClient = () => {
    ...
    const newClient: ClientItem = {
      ...
      activity: [{ type: "create", text: "Client created and onboarded into pipeline", who: "You", ts: Date.now() }],
    };
      ...
      <div className="flex flex-col h-[calc(100vh-150px)] min-h-[500px] overflow-hidden bg-[#F8FAFC] text-[#1F2937] font-sans">
        ...
        <div className="flex items-center gap-2 mb-4">
          <button
            ...
            className="flex items-center gap-1.5 bg-transparent border-0 text-[#475569] hover:text-[#1F2937] text-sm font-semibold cursor-pointer p-0 transition-colors"
          >
          ...
          <span className="text-sm font-bold text-[#1F2937]">{c.name}</span>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-[#E2E8F0] border-l-4 border-l-[#4F46E5] p-5 sm:p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4 mb-5">
          <div>
            <div className="flex items-center gap-3 mb-3.5">
              ...
              <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-[#F1F5F9] text-[#64748B]">
                {c.type} Dealership
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-6 sm:gap-7">
              <div>
                <div className="text-[11px] font-semibold text-[#94A3B8] mb-1">Owner</div>
                <div className="flex items-center gap-2">
                  <div
                    className="w-6 h-6 rounded-full text-white flex items-center justify-center text-[11px] font-bold"
                    style={{ backgroundColor: OWNER_COLOR[c.owner] || "#4F46E5" }}
                  >
                    {c.owner.slice(0, 1)}
                  </div>
                  <span className="text-sm font-bold text-[#1F2937]">{c.owner}</span>
                </div>
              </div>

              <div>
                <div className="text-[11px] font-semibold text-[#94A3B8] mb-1">Onboarded</div>
                <div className="text-sm font-bold text-[#1F2937]">{c.onboard}</div>
              </div>

              <div className="hidden sm:block w-px h-8 bg-[#E2E8F0]"></div>

              <div>
                <div className="text-[11px] font-semibold text-[#94A3B8] mb-1">Client contact</div>
                <div className="flex items-center gap-2.5">
                  <span className="text-sm font-bold text-[#1F2937]">{c.contact}</span>
                  <span className="inline-flex items-center gap-1 text-xs text-[#6366F1] font-semibold">
                    📞 {c.phone}
                  </span>
                </div>
              </div>
          ...
          <div>
            {isDue && (
              <div className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-[#FEF9C3] border border-[#FDE047]">
                ...
                <div>
                  <div className="text-sm font-extrabold text-[#854D0E]">Due Today</div>
                  <div className="text-xs font-semibold text-[#A16207]">Day {c.day}</div>
                </div>
              </div>
            )}
            {isOverdue && (
              <div className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-[#FEE2E2] border border-[#FCA5A5]">
                ...
                <div>
                  <div className="text-sm font-extrabold text-[#DC2626]">Overdue</div>
                  <div className="text-xs font-semibold text-[#B91C1C]">Day {c.day} · 2d over SLA</div>
                </div>
              </div>
            )}
            {isBlocked && (
              <div className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-[#FEE2E2] border border-[#FCA5A5]">
                <span className="text-xl">🚫</span>
                <div>
                  <div className="text-sm font-extrabold text-[#DC2626]">Blocked</div>
                  <div className="text-xs font-semibold text-[#B91C1C]">Day {c.day} · Action needed</div>
                </div>
              </div>
            )}
            {isLive && (
              <div className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-[#DCFCE7] border border-[#86EFAC]">
                <span className="text-xl">✓</span>
                <div>
                  <div className="text-sm font-extrabold text-[#16A34A]">Live</div>
                  <div className="text-xs font-semibold text-[#15803D]">{c.goLive}d Go-Live</div>
                </div>
              </div>
            )}
            {!isDue && !isOverdue && !isBlocked && !isLive && (
              <div className="flex items-center gap-2.5 px-5 py-3 rounded-xl bg-[#EEF2FF] border border-[#C7D2FE]">
                <span className="text-xl">⚡</span>
                <div>
                  <div className="text-sm font-extrabold text-[#4F46E5]">On Track</div>
                  <div className="text-xs font-semibold text-[#4338CA]">Day {c.day} · Within SLA</div>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
          ...
          <section className="lg:col-span-5 bg-white border border-[#E2E8F0] rounded-2xl shadow-2xs p-5 sm:p-6">
            <div className="flex items-center justify-between mb-2">
              <h2 className="m-0 text-sm font-bold text-[#1F2937]">Setup steps</h2>
              ...
              {STEP_NAMES.map((name, i) => {
                ...
                return (
                  <div key={name} className={`relative flex gap-3.5 ${isLast ? "pb-0" : "pb-4"}`}>
                    ...
                    {isDone ? (
                      <div className="w-7 h-7 rounded-full bg-[#16A34A] flex items-center justify-center shrink-0 text-white text-xs font-bold z-10">
                        ✓
                      </div>
                    ) : isActive ? (
                      <div className="w-7 h-7 rounded-full bg-[#EEF2FF] border-2 border-[#4F46E5] flex items-center justify-center shrink-0 text-[#4F46E5] text-xs font-bold z-10">
                        {i + 1}
                      </div>
                    ) : (
                      <div className="w-7 h-7 rounded-full bg-[#F1F5F9] border border-[#E2E8F0] flex items-center justify-center shrink-0 text-[#94A3B8] text-[11px] font-semibold z-10">
                        {i + 1}
                      </div>
                    )}
          ...
          <div className="lg:col-span-7 flex flex-col gap-5">
            <section className="bg-white border border-[#E2E8F0] rounded-2xl shadow-2xs overflow-hidden">
              <div className="p-5 pb-3.5">
                <div className="flex items-center justify-between">
                  <h2 className="m-0 text-sm font-bold text-[#1F2937]">Requirements</h2>
                  <span className="text-xs text-[#94A3B8]">click a slot to change status</span>
                </div>
                  ...
                  return (
                    <div key={category.cat} className="border-t border-[#F1F5F9]">
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <span
                            className={`text-xs text-[#64748B] transition-transform duration-150 inline-block ${
                              isOpen ? "rotate-90" : "rotate-0"
                            }`}
                          >
                            →
                          </span>
                          <span className="text-sm font-semibold text-[#1F2937]">{category.cat}</span>
                        </div>

                        <div>
                          {allReceived ? (
                            <span className="text-xs font-bold text-[#16A34A]">All received</span>
                          ) : (
                            <span className="text-xs font-bold text-[#DC2626]">{pendingCount} pending</span>
                          )}
                        </div>
                      </div>
                        ...
                        <div className="px-5 pl-10 pb-3 bg-[#F8FAFC]">
                          {category.items.map((itm) => {
                            ...
                            return (
                              ...
                              >
                                <span className="text-xs font-medium text-[#334155]">{itm.name}</span>
                                <div className="flex items-center gap-2">
                                  <button
                                    onClick={() => handleToggleReqStatus(category.cat, itm.name)}
                                    className={`px-2.5 py-0.5 rounded-full text-[10.5px] font-bold cursor-pointer transition-all active:scale-95 ${
                                      isRec
                                  ...
                                  <button
                                    onClick={() => handleToggleReqStatus(category.cat, itm.name)}
                                    className={`px-2 py-0.5 rounded-md text-xs font-semibold cursor-pointer transition-all active:scale-95 ${
                                      isPend
            ...
            <section className="bg-white border border-[#E2E8F0] rounded-2xl shadow-2xs p-5">
              <div className="flex items-center justify-between mb-3.5">
                <h2 className="m-0 text-sm font-bold text-[#1F2937]">Activity &amp; follow-up</h2>
              </div>
              ...
              <div className="flex gap-2 mb-4">
                <input
                  ...
                  className="flex-1 h-9 border border-[#CBD5E1] rounded-lg px-3 text-xs text-[#1F2937] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#4F46E5]"
                />
                <button
                  onClick={() => handleAddLog("note")}
                  className="px-3.5 h-9 rounded-lg bg-[#4F46E5] hover:bg-[#4338CA] text-white text-xs font-semibold cursor-pointer transition-all shadow-2xs active:scale-95"
                >
                  Add note
                </button>
              </div>
                ...
                {c.activity.map((act, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1">
                      <div className="text-xs text-[#1F2937] font-medium">{act.text}</div>
        ...
        <div className="shrink-0 bg-white border-t border-[#E2E8F0] px-8 py-3 flex items-center gap-3 z-50 shadow-[0_-4px_12px_rgba(0,0,0,0.06)]">
          <span className="text-sm font-medium text-[#64748B] mr-auto">
          ...
          <button
            ...
            className="h-9 px-3.5 rounded-lg border border-[#CBD5E1] bg-white hover:bg-[#F8FAFC] text-[#1F2937] text-xs font-semibold cursor-pointer flex items-center gap-1.5 transition-all shadow-2xs active:scale-95"
          >
            📞 Log a call
          </button>
          ...
          <button
            ...
            className={`h-9 px-3.5 rounded-lg text-xs font-semibold cursor-pointer flex items-center gap-1.5 transition-all shadow-2xs active:scale-95 border ${
              c.blocked
          ...
          {!c.live && (
            <button
              ...
              className="h-9 px-4 rounded-lg bg-[#4F46E5] hover:bg-[#4338CA] text-white text-xs font-bold cursor-pointer flex items-center gap-1.5 transition-all shadow-sm shadow-[#4F46E5]/30 active:scale-95"
            >
  ...
  // ============================================================
  // RENDER: ALL CLIENTS TABLE VIEW
  // ============================================================
  return (
    <div className="p-7 min-h-full bg-[#F8FAFC] text-[#1F2937] font-sans">
      {/* HEADER */}
      <div className="flex items-end justify-between mb-5">
        <div>
          ...
          <p className="mt-1.5 text-sm text-[#475569]">
            Every dealership in HR Setu implementation, across all owners and stages.
          </p>
        </div>
        <button
          onClick={() => setShowAddModal(true)}
          className="h-10 px-4 rounded-lg bg-[#4F46E5] hover:bg-[#4338CA] text-white text-sm font-semibold cursor-pointer flex items-center gap-2 shadow-sm shadow-[#4F46E5]/30 transition-all active:scale-95"
        >
      ...
      {/* SUMMARY STRIP */}
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2.5 mb-4.5">
        {[
          { label: "Total Active", count: totalCount - liveCount, bg: "bg-white", fg: "text-[#1F2937]", border: "border-[#E2E8F0]" },
          { label: "On Track", count: onTrackCount, bg: "bg-white", fg: "text-[#16A34A]", border: "border-[#E2E8F0]" },
          { label: "Due Today", count: dueTodayCount, bg: "bg-white", fg: "text-[#B45309]", border: "border-[#E2E8F0]" },
          { label: "Overdue", count: overdueCount, bg: overdueCount > 0 ? "bg-[#FEF2F2]" : "bg-white", fg: "text-[#DC2626]", border: overdueCount > 0 ? "border-[#FCA5A5]" : "border-[#E2E8F0]" },
          { label: "Blocked", count: blockedCount, bg: blockedCount > 0 ? "bg-[#FEF2F2]" : "bg-white", fg: "text-[#DC2626]", border: blockedCount > 0 ? "border-[#FCA5A5]" : "border-[#E2E8F0]" },
          { label: "Live", count: liveCount, bg: "bg-white", fg: "text-[#16A34A]", border: "border-[#E2E8F0]" },
        ].map((s) => (
          <div key={s.label} className={`${s.bg} border ${s.border} rounded-xl p-3 sm:p-3.5 shadow-2xs`}>
            <div className="text-[11px] font-semibold text-[#64748B]">{s.label}</div>
            <div className={`text-xl font-bold ${s.fg} mt-0.5`}>{s.count}</div>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <div className="relative w-full sm:w-72">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg>
          <input
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            placeholder="Search clients…"
            className="w-full h-9.5 border border-[#E2E8F0] rounded-lg bg-white pl-9 pr-3 text-xs text-[#1F2937] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#4F46E5]"
          />
        </div>
        ...
        <select
          ...
          className="h-9.5 border border-[#E2E8F0] rounded-lg bg-white px-3 text-xs text-[#1F2937] cursor-pointer focus:outline-none focus:border-[#4F46E5]"
        >
      ...
      {/* DATA TABLE */}
      <section className="bg-white border border-[#E2E8F0] rounded-2xl shadow-2xs overflow-hidden">
        <div className="flex items-center gap-4 px-5 py-3 bg-[#F8FAFC] border-b border-[#F1F5F9] text-[11px] font-bold tracking-wider uppercase text-[#94A3B8]">
          <span className="w-52">Client</span>
        ...
        {filteredClients.length === 0 ? (
          <div className="text-center py-10 px-5 text-[#94A3B8] text-sm">
          ...
          filteredClients.map((c) => {
            ...
            const stepCount = c.live ? 11 : c.idx + 1;
            const progressPercent = Math.min(100, Math.round((stepCount / 11) * 100));
            const ownerBg = OWNER_COLOR[c.owner] || "#4F46E5";

            return (
              <div
                key={c.id}
                onClick={() => setSelectedClient(c)}
                className="flex items-center gap-4 px-5 py-3.5 border-b border-[#F1F5F9] text-xs cursor-pointer hover:bg-[#F8FAFC] transition-colors"
              >
                <div className="w-52">
                  <div className="font-bold text-[#1F2937] text-sm">{c.name}</div>
                  <div className="text-[11px] text-[#64748B] mt-0.5">{c.city}</div>
                </div>
                ...
                <div className="w-32 flex items-center gap-2">
                  <div
                    className="w-5.5 h-5.5 rounded-full text-white flex items-center justify-center text-[10px] font-bold"
                    style={{ backgroundColor: ownerBg }}
                  >
                    {c.owner.slice(0, 1)}
                  </div>
                  <span className="font-semibold text-[#334155]">{c.owner}</span>
                </div>

                <div className="w-48 text-xs text-[#334155]">
                  {c.live ? "11 · Completed & Live" : `${c.idx + 1} · ${STEP_NAMES[c.idx] || "Done"}`}
                </div>

                <div className="w-40">
                  <div className="flex justify-between text-[11px] font-semibold text-[#64748B] mb-1">
                    <span>{stepCount}/11 steps</span>
                    <span>{progressPercent}%</span>
                  </div>
                  <div className="w-full h-1.5 bg-[#E2E8F0] rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${c.live ? "bg-[#16A34A]" : "bg-[#4F46E5]"}`}
                ...
                <div className="flex-1">
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-bold ${slaBg} ${slaFg}`}
                  >
                    {slaLabel}
                  </span>
                </div>
                ...
                <div
                  className={`w-16 text-right font-bold ${
                    isOverdue || isBlocked ? "text-[#DC2626]" : isLive ? "text-[#16A34A]" : "text-[#475569]"
                  }`}
                >
                  {c.live ? (c.goLive ? `${c.goLive}d` : "Live") : `Day ${c.day}`}
                </div>
              </div>
            );
          })
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 bg-[#0F172A]/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xl overflow-hidden flex flex-col shadow-2xl border border-[#E2E8F0] animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 space-y-5 overflow-y-auto max-h-[90vh]">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full h-11 px-3.5 rounded-xl border border-[#E2E8F0] bg-white text-sm text-[#0F172A] placeholder:text-[#94A3B8] focus:outline-none focus:ring-2 focus:ring-[#4338CA]/20 focus:border-[#4338CA] transition-all"
                  />
                </div>
                  ...
                  <div>
                    ...
                    <div className="h-11 bg-[#F1F5F9] p-1 rounded-xl flex items-center gap-1">
                      <button
                        ...
                        className={`flex-1 h-full rounded-lg text-sm font-semibold flex items-center justify-center transition-all cursor-pointer ${
                          formData.type === "4W"
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full h-11 px-3.5 pr-10 rounded-xl border border-[#E2E8F0] bg-white text-sm font-medium text-[#0F172A] focus:outline-none focus:ring-2 focus:ring-[#4338CA]/20 focus:border-[#4338CA] transition-all cursor-pointer appearance-none"
                      >
                        <option value="Bhoomika">Bhoomika</option>
                        <option value="Prachi">Prachi</option>
                        <option value="Chirag">Chirag</option>
              ...
              <div className="flex items-center justify-end gap-3 pt-2">
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl border border-[#E2E8F0] bg-white hover:bg-[#F8FAFC] text-[#334155] text-sm font-semibold transition-all cursor-pointer shadow-2xs"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-2.5 rounded-xl bg-[#4338CA] hover:bg-[#3730A3] text-white text-sm font-semibold transition-all cursor-pointer shadow-sm shadow-[#4338CA]/30 active:scale-95"
                >
                  Create client
                </button>
              </div>
