...
export default function OnboardingBoardPage() {
  ...
  // Modals & State inside Client Detail
  const [showAddModal, setShowAddModal] = useState(false);
  const [openReqCats, setOpenReqCats] = useState<Record<string, boolean>>({
    Company: true,
    Employee: true,
    Salary: true,
  });
  ...
  const columns = [
    { title: "Requirements", idx: 0 },
    { title: "Data Restore", idx: 1 },
    { title: "Company / Godown Master", idx: 2 },
    { title: "Shift Master", idx: 3 },
    { title: "Employee Import", idx: 4 },
    { title: "Salary Structure", idx: 5 },
    { title: "Machine Integration", idx: 6 },
    { title: "Misspunch / Leave", idx: 7 },
    { title: "Policy Setup", idx: 8 },
    { title: "User Rights", idx: 9 },
    { title: "Attendance & Salary Re-run", idx: 10 },
    { title: "Done", idx: 99, done: true },
  ];
  ...
  // ============================================================
  // RENDER: CLIENT DETAIL VIEW (MATCHING USER SCREENSHOT)
  // ============================================================
  if (selectedClient) {
  ...
  return (
    <div className="p-6 sm:p-7 min-h-full bg-[#F8FAFC] text-[#1F2937] font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        <div>
          <div className="text-lg font-bold tracking-wider uppercase text-[#6366F1] mb-1">
            BOARD VIEW
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="relative w-full sm:w-60">
            <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 pointer-events-none" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#94A3B8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="7"/><path d="M21 21l-4.3-4.3"/></svg>
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search clients..."
              className="w-full h-9 border border-[#E2E8F0] rounded-lg bg-white pl-8 pr-2.5 text-lg text-[#1F2937] placeholder:text-[#94A3B8] focus:outline-none focus:border-[#4F46E5]"
            />
          </div>
          ...
          <select
            ...
            className="h-9 border border-[#E2E8F0] rounded-lg bg-white px-2.5 text-lg text-[#1F2937] cursor-pointer focus:outline-none focus:border-[#4F46E5]"
          >
          ...
          <button
            ...
            className="h-9 px-3.5 rounded-lg bg-[#4F46E5] hover:bg-[#4338CA] text-white text-lg font-semibold cursor-pointer flex items-center gap-1.5 shadow-sm shadow-[#4F46E5]/30 transition-all active:scale-95"
          >
      ...
      <div className="overflow-x-auto pb-4">
        <div className="flex gap-4 min-w-max items-start">
          {columns.map((col) => {
            ...
            const colCards = filteredClients.filter((c) => {
              if (isDoneCol) return c.live || c.complete;
              return !c.live && !c.complete && c.idx === col.idx;
            });

            return (
              <div
                key={col.title}
                className="w-64 shrink-0 bg-[#F1F5F9] rounded-2xl border border-[#E2E8F0] flex flex-col max-h-[calc(100vh-200px)] shadow-2xs"
              >
                {/* Column Header */}
                <div
                  className={`p-3 px-3.5 border-b border-[#E2E8F0] flex items-center justify-between rounded-t-2xl ${
                    isDoneCol ? "bg-[#F0FDF4]" : "bg-white"
                  }`}
                >
                  <div className="flex items-center gap-1.5">
                    <div
                      className={`w-2 h-2 rounded-full ${isDoneCol ? "bg-[#16A34A]" : "bg-[#6366F1]"}`}
                    ></div>
                    <span className="text-lg font-bold text-[#1F2937]">{col.title}</span>
                  </div>
                  <span
                    className={`text-lg font-bold px-2 py-0.5 rounded-full ${
                      isDoneCol ? "bg-[#DCFCE7] text-[#16A34A]" : "bg-[#E2E8F0] text-[#475569]"
                    }`}
                  >
                    {colCards.length}
                  </span>
                ...
                <div className="p-2.5 overflow-y-auto flex flex-col gap-2.5 flex-1">
                  {colCards.length === 0 ? (
                    <div className="text-center py-6 px-2 text-[#94A3B8] text-lg italic">
                    ...
                    colCards.map((c) => {
                      ...
                      return (
                        <div
                          ...
                          className={`bg-white rounded-xl p-3 border shadow-xs cursor-pointer hover:shadow-sm transition-all ${
                            isOverdue
                              ? "border-[#FCA5A5] border-l-3 border-l-[#DC2626]"
                              : isBlocked
                              ? "border-[#FCA5A5] border-l-3 border-l-[#DC2626]"
                        ...
                        >
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <div className="text-xl font-bold text-[#1F2937] leading-tight">
                                {c.name}
                              </div>
                            ...
                            <div className="flex items-center gap-1">
                              {isBlocked && (
                                <span title="Blocked" className="text-[#DC2626] inline-flex">
                                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
                                </span>
                              )}
                              <span
                          ...
                          <div className="flex items-center justify-between mt-3 pt-2 border-t border-[#F1F5F9]">
                            ...
                            {c.live ? (
                              <span className="text-[15px] font-bold px-1.5 py-0.5 rounded-md bg-[#DCFCE7] text-[#16A34A]">
                                {c.goLive ? `${c.goLive}d` : "Live"}
                              </span>
                            ) : (
                              <span
                                className={`text-[15px] font-bold px-1.5 py-0.5 rounded-md ${
                                  isOverdue || isBlocked
                                    ? "bg-[#FEE2E2] text-[#DC2626]"
                                    : isDue
                                    ? "bg-[#FEF3C7] text-[#B45309]"
