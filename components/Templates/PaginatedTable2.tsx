...
const PaginatedTable = ({
  ...
  const tableData = useMemo(() => {
    return filterPosition === "FilterData" ? filteredData : data;
  }, [filteredData, data, filterPosition]);
  ...
  const onclickExceldownload = () => {
    handleExcelDownload(columnsDownload || columns, tableData);
  };
  ...
  return (
    <div className="p-0">
      <div className="h-12 px-4 mt-2">
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-2 mt-0">
            ...
            <Button className="hidden md:block w-[126px]" onClick={onclickExceldownload} size={'sm'} variant={"save"}
            ...
            <input
              className="border-0 px-1.5 w-52 ring-2 text-black dark:text-white py-1.5 dark:bg-input rounded text-sm shadow-xl focus:outline-none focus:ring ease-linear transition-all duration-150"
              type="text"
              value={globalFilter || ""}
      ...
      <div className={`overflow-y-scroll h-[${height}] no-visible-scrollbar`}>
        <table
          {...getTableProps()}
          ref={tableRef}
        ...
        >
          ...
          {/* Main table header */}
          <thead
            className={`sticky ${filterPosition === "FilterData" && enableColumnFilters ? "top-[38px]" : "top-0"} z-1 border-b border-body-color h-6 bg-[#193A69] dark:bg-input text-white dark:text-white`}
                ...
                >
                  {row.cells.map((cell) => (
                    <td
                      key={cell.column.id}
                      {...cell.getCellProps()}
                      className={`border text-left px-2 border-body-color py-2 text-ellipsis whitespace-nowrap ${cell.column.align === "right"
                        ? "text-right"
