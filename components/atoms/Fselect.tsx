...
const Fselect = ({
  ...
  useEffect(() => {
    // Update selected option when initialValue changes
    const selected = option?.find(opt => opt.value?.toString()?.toUpperCase() == initialValue?.toString()?.toUpperCase());
    setSelectedOption(selected || null);
    handleSelect(selected)
    if (setFirst) {
      setSelectedOption(option[0]);
      handleInputChange(name, option[0]?.value);
      handleLabelChange(name, option[0]?.label);
      handleChangeData(name, option[0]);
    }
  ...
  function handleSelect(selectedOption) {
    ...
    handleInputChange(name, selectedOption?.value || option?.map((options) => options.value)?.join(',')); // Update form data
  ...
  return (
    <div className='w-full'>
      {title && (
        <label
          className={`flex font-bold mt-1 mb-1 text-header dark:text-white ${labelClass || "text-xs"}`}
          htmlFor={name}
        >
          {toTitleCase(title)}
        </label>
      ...
      <div>
        ...
        <Select
          ...
          options={(isSelectAll && option.length > 1) ? [{ value: option?.map((options) => options.value)?.join(','), label: `ALL ${title?.toUpperCase()}` }, ...option] : option}
