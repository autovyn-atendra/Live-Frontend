...
const Iselect = ({
  ...
  useEffect(() => {
    if (listRef.current && containerRef.current) {
      ...
      console.log(listRect.height, "listRect.height");
      if (spaceBelow < spaceAbove && spaceAbove > listRect.height) {
        setDropup(true);
      } else {
        setDropup(false);
      }
  ...
  return (
    <div
      className={`w-full font-sm h-5 flex flex-col -mt-1 gap-1 relative ${
        disabled ? "opacity-50" : ""
      }`}
      ref={containerRef}
    >
      <label className="flex capitalize  " htmlFor={name}>
        {title?.toLowerCase()}
      ...
      <div
        ...
        className={`w-full flex  items-center border border-borderColor-dark h-${
          h || 6
        } p-4  text-lg justify-between rounded dark:bg-input bg-white  cursor-pointer ${
